...
router.get('/v1/games/:game/settings', function(req, res) {
    ...
    db.get(`SELECT * FROM games WHERE code='${req.params.game}'`, (err, row) => {
        ...
        let user_id = req.session.auth_result.user.id;
        db.run(`INSERT INTO game_sessions(game_id,user_id) VALUES(${game.id},${user_id})`, function(err) {
            if (err) {
...
router.post('/v1/games/:game', function(req, res) {
  ...
  db.run(`UPDATE game_sessions SET accuracy = ${req.body.accuracy || 0}, reaction_time = ${req.body.reactionTime || 0}, score = ${req.body.score}, total_correct = ${req.body.totalCorrect}, total_incorrect = ${req.body.totalIncorrect}, completed_at = DATETIME('NOW', 'LOCALTIME') WHERE id = ${req.body.session_id}`, (err) => {
    if (err) {
...
router.get('/v1/games/statistic/:session_id', function(req, res) {
  console.log(`load '${req.params.session_id}' statistics`);
  db.get(`SELECT * FROM game_sessions WHERE id='${req.params.session_id}'`, (err, row) => {
    if (err) {
    ...
    if (row.user_id != req.session.auth_result.user.id) {
